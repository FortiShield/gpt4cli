...
func executor(in string, p *prompt.Prompt) {
	...
	if strings.HasPrefix(lastLine, "gpt4cli ") || strings.HasPrefix(lastLine, "g4c ") {
		...
		if len(parts) > 1 {
			...
			_, err := lib.ExecGpt4cliCommandWithParams(args, lib.ExecGpt4cliCommandParams{
				SessionId: sessionId,
			})
			if err != nil {
	...
	if lib.CurrentReplState.Mode == lib.ReplModeTell {
		...
		output, err := lib.ExecGpt4cliCommandWithParams(args, lib.ExecGpt4cliCommandParams{
			SessionId: sessionId,
		})
		if err != nil {
...
func execWithInput(params execWithInputParams) execWithInputResult {
	...
	default:
		...
		if matchedCmd != "" {
			...
			_, err := lib.ExecGpt4cliCommandWithParams(execArgs, lib.ExecGpt4cliCommandParams{
				SessionId: sessionId,
			})
			if err != nil {
				color.New(term.ColorHiRed).Printf("Error executing command: %v\n", err)
