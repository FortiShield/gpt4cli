...
func listAvailableModels(cmd *cobra.Command, args []string) {
	...
	if customModelsOnly && auth.Current.IntegratedModelsMode {
		term.OutputErrorAndExit("Custom models are not supported in Integrated Models mode on Gpt4cli Cloud")
		return
	}
...
func customModelsNotImplemented(cmd *cobra.Command, args []string) {
	...
	fmt.Println()
	fmt.Println("Use " + color.New(color.BgCyan, color.FgHiWhite).Sprint(" gpt4cli models custom ") + " to manage custom models, providers, and model packs")
	os.Exit(1)
}
