...
func listProviders(cmd *cobra.Command, args []string) {
	...
	if customProvidersOnly && auth.Current.IsCloud {
		term.OutputErrorAndExit("Custom providers are not supported on Gpt4cli Cloud")
		return
	}
	...
	for _, p := range shared.AllModelProviders {
		...
		if config.Provider == shared.ModelProviderAmazonBedrock {
			extraVars = append(extraVars, "GPT4CLI_AWS_PROFILE")
		}
	...
	fmt.Println("Go to → " + color.New(color.Bold).Sprint("https://docs-gpt4cli.khulnasoft.com/models/model-providers"))
	fmt.Println()
