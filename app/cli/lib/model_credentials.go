...
func ResolveProviderAuthVars(cfg *shared.ModelProviderConfigSchema) (map[string]string, error) {
	...
	if cfg.HasAWSAuth {
		// GPT4CLI_AWS_PROFILE enables credential file loading from ~/.aws/credentials
		// this ensures it's opt-in so we only use bedrock if user explicitly intends to
		profile := os.Getenv("GPT4CLI_AWS_PROFILE")
		if profile != "" {
		...
		// if no GPT4CLI_AWS_PROFILE is set OR loading aws vars fails, just silently fall through to the default env var checks
...
func showCredentialErrorMessage(res CredentialCheckResult, opts shared.ModelProviderOptions) {
	...
	if allPublishersHaveOpenRouter {
		...
		if term.IsRepl {
			...
			step(6, "Restart the REPL with "+cyanChip.Sprint(" gpt4cli "))
		} else {
	...
	if len(byPub) > 0 {
		...
		fmt.Println("For details on the API key/credentials required for each provider, go to:\n" + color.New(color.Bold).Sprint("https://docs-gpt4cli.khulnasoft.com/models/model-providers"))
	}
