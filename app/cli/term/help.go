...
var CliCommands = []CmdConfig{
	{"", "", "start the Gpt4cli REPL", false},

	// {"--full", "", fmt.Sprintf("start the Gpt4cli REPL with auto-mode %s", "'full'"), false},
	// {"--semi", "", fmt.Sprintf("start the Gpt4cli REPL with auto-mode %s", "'semi'"), false},
	// {"--plus", "", fmt.Sprintf("start the Gpt4cli REPL with auto-mode %s", "'plus'"), false},
	// {"--basic", "", fmt.Sprintf("start the Gpt4cli REPL with auto-mode %s", "'basic'"), false},
	// {"--none", "", fmt.Sprintf("start the Gpt4cli REPL with auto-mode %s", "'none'"), false},

	// {"--daily", "", fmt.Sprintf("start the Gpt4cli REPL with %s model pack", "'daily-driver'"), false},
	// {"--strong", "", fmt.Sprintf("start the Gpt4cli REPL with %s model pack", "'strong'"), false},
	// {"--cheap", "", fmt.Sprintf("start the Gpt4cli REPL with %s model pack", "'cheap'"), false},
	// {"--oss", "", fmt.Sprintf("start the Gpt4cli REPL with %s model pack", "'oss'"), false},
	...
	{"usage", "", "show Gpt4cli Cloud current balance and usage report", true},
	{"usage --today", "", "show Gpt4cli Cloud usage for the day so far", true},
	{"usage --month", "", "show Gpt4cli Cloud usage for the current billing month", true},
	{"usage --plan", "", "show Gpt4cli Cloud usage for the current plan", true},

	{"usage --log", "", "show Gpt4cli Cloud transaction log", true},

	{"billing", "", "show Gpt4cli Cloud billing settings", true},
}
...
func printCmds(w io.Writer, prefix string, colors []color.Attribute, cmds ...string) {
	if os.Getenv("GPT4CLI_DISABLE_SUGGESTIONS") != "" {
		return
	}
	...
	for _, cmd := range cmds {
		...
		if IsRepl {
			...
			styled = color.New(colors...).Sprintf(" gpt4cli ")
		} else {
			styled = color.New(colors...).Sprintf(" gpt4cli %s ", cmd)
		}
...
func PrintCustomCmd(prefix, cmd, alias, desc string) {
	cmd = strings.Replace(cmd, alias, fmt.Sprintf("(%s)", alias), 1)
	// desc += color.New(color.FgWhite).Sprintf(" • alias → %s", fmt.Sprint(a'lias'))
	styled := color.New(color.Bold, color.FgHiWhite, color.BgCyan, color.FgHiWhite).Sprintf(" gpt4cli %s ", cmd)
	fmt.Printf("%s%s 👉 %s\n", prefix, styled, desc)
}

// PrintCustomHelp prints the custom help output for the Gpt4cli CLI
func PrintCustomHelp(all bool) {
	...
	color.New(color.Bold).Fprintln(builder, "  gpt4cli help # show basic usage")
	color.New(color.Bold).Fprintln(builder, "  gpt4cli help --all # show all commands")
	color.New(color.Bold).Fprintln(builder, "  gpt4cli [command] --help")
	fmt.Fprintln(builder)

	color.New(color.Bold, color.BgMagenta, color.FgHiWhite).Fprintln(builder, " Getting Started ")
	fmt.Fprintln(builder)
	fmt.Fprintf(builder, " 🚀 Start the Gpt4cli REPL in a project directory with %s or %s\n", color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" gpt4cli "), color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" g4c "))
	fmt.Fprintln(builder)
	fmt.Fprintf(builder, " 💻 You can also use any command outside the REPL with %s or %s\n", color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" gpt4cli [command] "), color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" g4c [command] "))
	fmt.Fprintln(builder)
	...
	if all {
		...
		color.New(color.Bold, color.BgHiBlue, color.FgHiWhite).Fprintln(builder, " Use 'gpt4cli help --all' or 'gpt4cli help -a' for a list of all commands ")
		fmt.Fprintln(builder)
