	...
	if len(modelsInput.CustomModels) > 0 {
		if os.Getenv("IS_CLOUD") != "" {
			...
			if apiOrg.IntegratedModelsMode {
				http.Error(w, "Custom models are not supported on Gpt4cli Cloud in Integrated Models mode", http.StatusBadRequest)
				return
			}
		}
	}
		...
		for _, provider := range model.Providers {
			if provider.Provider == shared.ModelProviderCustom {
				...
				if os.Getenv("IS_CLOUD") != "" && pc.LocalOnly {
					msg := fmt.Sprintf("'%s' is a local-only model provider, so it can't be used on Gpt4cli Cloud", provider.Provider)
					log.Println(msg)
					http.Error(w, msg, http.StatusUnprocessableEntity)
					return
				}
	...
	for _, modelPack := range updatedModelsInput.CustomModelPacks {
		...
		for _, modelId := range allModelIds {
			...
			if builtIn && os.Getenv("IS_CLOUD") != "" && bm.IsLocalOnly() {
				msg := fmt.Sprintf("'%s' is a local-only built-in model, so it can't be used on Gpt4cli Cloud", modelId)
				log.Println(msg)
				http.Error(w, msg, http.StatusUnprocessableEntity)
				return
			}
