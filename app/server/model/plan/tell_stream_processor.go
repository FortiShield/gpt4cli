...
var openingTagRegex = regexp.MustCompile(`<Gpt4cliBlock\s+lang="(.+?)"\s+path="(.+?)".*?>`)
...
func (state *activeTellStreamState) processChunk(choice types.ExtendedChatCompletionStreamChoice) processChunkResult {
	...
	if processor.fileOpen && strings.HasSuffix(active.CurrentReplyContent+content, "</Gpt4cliBlock>") {
		if verboseLogging {
...
func (processor *chunkProcessor) bufferOrStream(content string, parserRes *types.ReplyParserRes, currentStage shared.CurrentStage, manualStopSequences []string) bufferOrStreamResult {
	...
	if awaitingTag {
		...
		if processor.awaitingBlockOpeningTag {
			...
			if parserRes.CurrentFilePath != "" {
				...
				if matched {
					...
					if verboseLogging {
						log.Printf("Opening <Gpt4cliBlock> tag is missing even though parserRes.CurrentFile is set - something is wrong: %s\n", content)
					}
					processor.awaitingBlockOpeningTag = false
					processor.fileOpen = false
				...
				if len(split) > 1 {
					...
					if strings.HasPrefix(`Gpt4cliBlock lang="`, last) {
						if verboseLogging {
							log.Println("strings.HasPrefix(`Gpt4cliBlock lang=", last)
						}
						shouldStream = false
						matchedPrefix = true
					} else if strings.HasPrefix(last, `Gpt4cliBlock lang="`) {
						if verboseLogging {
			...
			if parserRes.CurrentFilePath == "" {
				if strings.Contains(content, "</Gpt4cliBlock>") {
					shouldStream = true
					processor.awaitingBlockClosingTag = false
					processor.fileOpen = false
					// replace </Gpt4cliBlock> with ``` to close the markdown code block
					content = strings.ReplaceAll(content, "</Gpt4cliBlock>", "```")
				} else {
					log.Printf("Closing </Gpt4cliBlock> tag is missing even though parserRes.CurrentOperation is nil - something is wrong: %s\n", content)
					processor.awaitingBlockClosingTag = false
					shouldStream = true
				}
			}
			...
			if strings.Contains(content, "<EndGpt4cliFileOps/>") {
				if verboseLogging {
					log.Printf("Found <EndGpt4cliFileOps/>\n")
				}
				processor.awaitingOpClosingTag = false
				content = strings.Replace(content, "\n<EndGpt4cliFileOps/>", "", 1)
				content = strings.Replace(content, "<EndGpt4cliFileOps/>", "", 1)
				shouldStream = true
			}
		}
		...
		if parserRes.MaybeFilePath != "" && parserRes.CurrentFilePath == "" {
			...
			// this will set processor.awaitingBlockOpeningTag to true if the content starts with any prefix of<Gpt4cliBlock lang=" *or* any prefix of a full opening tag
		...
		if parserRes.CurrentFilePath != "" {
			...
			if strings.Contains(content, "</Gpt4cliBlock>") {
				if verboseLogging {
					log.Println("strings.Contains(content, \"</Gpt4cliBlock>\")")
				}
				processor.awaitingBlockClosingTag = true
			} else {
				if verboseLogging {
					log.Println("not strings.Contains(content, \"</Gpt4cliBlock>\")")
				}
				split := strings.Split(content, "<")
				// log.Printf("split: %v\n", split)
				if len(split) > 1 {
					...
					last := split[len(split)-1]
					// log.Printf("last: %s\n", last)
					if strings.HasPrefix("/Gpt4cliBlock>", last) {
						if verboseLogging {
							log.Println("strings.HasPrefix(\"/Gpt4cliBlock>\", last)")
						}
						processor.awaitingBlockClosingTag = true
					}
				}
			...
			if strings.Contains(content, "<EndGpt4cliFileOps/>") {
				if verboseLogging {
					log.Println("strings.Contains(content, \"<EndGpt4cliFileOps/>\")")
				}
				processor.awaitingOpClosingTag = true
			} else {
				if verboseLogging {
					log.Println("not strings.Contains(content, \"<EndGpt4cliFileOps/>\")")
				}
				split := strings.Split(content, "<")
				if len(split) > 1 {
					if verboseLogging {
					...
					last := split[len(split)-1]
					if strings.HasPrefix("EndGpt4cliFileOps/>", last) {
						if verboseLogging {
							log.Println("strings.HasPrefix(\"EndGpt4cliFileOps/>\", last)")
						}
						processor.awaitingOpClosingTag = true
					}
				}
			}
		} else if strings.Contains(content, "</Gpt4cliBlock>") {
			if verboseLogging {
				log.Println("strings.Contains(content, \"</Gpt4cliBlock>\")")
			}
			content = strings.Replace(content, "</Gpt4cliBlock>", "```", 1)
		} else if strings.Contains(content, "<EndGpt4cliFileOps/>") {
			if verboseLogging {
				log.Println("strings.Contains(content, \"<EndGpt4cliFileOps/>\")")
			}
			content = strings.Replace(content, "\n<EndGpt4cliFileOps/>", "", 1)
...
func replaceCodeBlockOpeningTag(content string, replaceWithFn func(lang string) string) (bool, string) {
	// check for opening tag matching <Gpt4cliBlock lang="..." path="...">
	...
	if match != nil {
		...
		return true, strings.Replace(content, "<Gpt4cliBlock>", replaceWithFn(""), 1)
	}
