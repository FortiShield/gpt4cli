...
func (fileState *activeBuildStreamFileState) buildValidate(
	...
	if params.validateOnly {
		stop = []string{"<Gpt4cliComments>", "<Gpt4cliReplacements>"}
	}
...
func handleXMLResponse(
	...
	if strings.Contains(content, "<Gpt4cliCorrect/>") {
	...
	replacementsOuter := utils.GetXMLContent(content, "Gpt4cliReplacements")
	...
	for i, replacement := range replacements {
		...
		firstLineNum, err := shared.ExtractLineNumberWithPrefix(firstLine, "g4c-")
		if err != nil {
		...
		if lastLine != "" {
			lastLineNum, err = shared.ExtractLineNumberWithPrefix(lastLine, "g4c-")
			if err != nil {
	...
	if strings.Contains(content, "<Gpt4cliIncorrect/>") {
		split := strings.Split(content, "<Gpt4cliIncorrect/>")
		problem = split[0]
	} else if strings.Contains(content, "<Gpt4cliReplacements>") {
		split := strings.Split(content, "<Gpt4cliReplacements>")
		problem = split[0]
	}
